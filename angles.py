...
def get_t1_t5_list(strokes, timestamps_arr):
    t_1_t_5_list = []
    for stroke in strokes:
        generated_velocity = np.zeros_like(timestamps_arr)
        generated_velocity += utilities.generate_lognormal_curve(stroke[0],
                                                                 stroke[1],
                                                                 stroke[2],
                                                                 stroke[3],
                                                                 timestamps_arr[0],
                                                                 timestamps_arr[-1],
                                                                 len(timestamps_arr))
        t_3 = timestamps_arr[np.argmax(generated_velocity)]
        t_1 = timestamps_arr[(generated_velocity < (0.01 * np.max(generated_velocity))) & (timestamps_arr < t_3)]
        t_5 = timestamps_arr[(generated_velocity < (0.01 * np.max(generated_velocity))) & (timestamps_arr > t_3)]
        if len(t_1) != 0:
            t_1 = t_1[-1]
        else:
            t_1 = timestamps_arr[0]
        if len(t_5) != 0:
            t_5 = t_5[0]
        else:
            t_5 = timestamps_arr[-1]
        t_1_t_5_list.append((t_1, t_5))
    return np.array(t_1_t_5_list)


def angle(x_point, timestampes, x_values, y_values):
    dx = np.gradient(x_values)
    dy = np.gradient(y_values)
    first_derevitive = dy / dx
    index = np.argmin(np.abs(timestampes - x_point))
    to_teturn = np.arctan(first_derevitive[index])
    return np.arctan(first_derevitive[index])



def start_and_end_angles(t1_t5, timestamps_arr, x_values, y_values):
    strokes_start_and_end_angles = []
    for t1, t5 in t1_t5:
        angle1 = angle(t1, timestamps_arr, x_values, y_values)
        angle5 = angle(t5, timestamps_arr, x_values, y_values)
        strokes_start_and_end_angles.append((angle1,angle5))
        print(np.degrees(angle(t1, timestamps_arr, x_values, y_values)))
        print(np.degrees(angle(t5, timestamps_arr, x_values, y_values)))
        plt.plot(x_values, y_values)
        plt.scatter(x_values[timestamps_arr==t1], y_values[timestamps_arr==t1], color="red")
        plt.scatter(x_values[timestamps_arr==t5], y_values[timestamps_arr==t5], color="purple")
        plt.show()
    return np.array(strokes_start_and_end_angles)

...
if __name__ == '__main__':
    ...
    D_1 = 40  # Amplitude range(5 -> 70)
    std_dev_1 = 0.45  # Standard deviation (sigma) range(0.1 -> 0.45)
    mean_1 = -1.8  # Mean (meu) range(-2.2 -> -1.6)
    x_0 = 0  # shifted range(0 -> 1)

    D_2 = 3  # Amplitude range(0.5 -> 7 ungefähr = 0.1*D_1)
    std_dev_2 = 0.3  # Standard deviation (sigma) range(0.1 -> 0.45)
    mean_2 = -2  # Mean (meu) range(-2.2 -> -1.6)
    ...
    # plt.plot(x_values, generated_velocity_aganostic, color="red")
    # plt.plot(x_values, generated_vilocity_antagonist, color="green")
    plt.plot(x_values, generated_velocity_profile, color="black")
    # plt.plot(x_values, calculated_vilocity_antagonist, color="red")
    plt.show()